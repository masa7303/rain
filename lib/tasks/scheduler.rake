...
task :update_feed => :environment do
  ...
  # 使用したxmlデータ
  url = "https://www.drk7.jp/weather/xml/16.xml"
  ...
  min_per = 20
  if per06to12.to.i >= min_per || per12to18.to._i >= min_per || per18to24.to_i >= min_per
    word1 =
      ["いい天気だね！",
       "今日もよく眠れた？",
       "二日酔い大丈夫？",
    ...
    # 降水確率によってメッセージを変更する設定
    mid_per = 50
    if per06to12.to.i >= mid_per || per12to18.to._i >= mid_per || per18to24.to_i >= mid_per
      word3 = "今日は雨が降りそうだから傘を忘れないでね！"
    else
      word4 = "今日は雨が降るかもしれないから折りたたみ傘があると安心だよ！"
    end
    # 発信するメッセージの設定
    push =
      "#{word1}\n#{word3}降水確率はこんな感じだよ。\n    6~12時　#{per06to12}％\n　12〜18時　 #{per12to18}％\n　18〜24時　#{per18to24}％\n#{word2}"
